...
- **チーム参加制御**: 個別チームの参加ON/OFF切り替えと自動マッチテーブル更新
- **スコア記録**: 直感的なクリック操作による試合スコア入力
- **順位表**: 勝率・得失点差・直接対戦結果による高精度な自動順位計算と表示
...
5. **順位確認**: 試合結果から自動計算される順位表を確認
   - 順位決定基準: ①勝率 ②得失点差 ③直接対戦結果 ④勝利数 ⑤得点合計

### 高度な機能
- **チーム編集**: チーム名横の編集ボタンでメンバー構成を変更可能
- **チーム参加制御**: チーム名横のトグルボタン（✓/✗）で参加状況を個別設定
  - 参加OFF（✗）のチームは対戦表と順位表から除外
  - 最小2チーム必要（不足時は警告表示）
  - 状態は自動保存・復元
...
#### 📊 **詳細分析レポート構成**
1.  **大会情報**: 名称、日付、場所、形式など（config.jsonから取得）
2.  **システム設定情報**: マッチポイント、エクスポート日時、アプリバージョン
3.  **大会統計サマリー**:
    *   総チーム数、総試合数、完了試合数、未実施試合数
    *   勝敗決着試合数、引き分け試合数
    *   進行率 (%)
    *   総得点数、1試合平均得点（チーム毎）
4.  **チーム構成メンバー一覧**: 各チームのID、チーム名、メンバー名
5.  **全試合結果一覧**: 対戦ID、対戦チーム情報（IDと名称）、各チームスコア、勝者チームID、試合結果（例: チームX勝利、引き分け）、試合状況（未実施/終了）
6.  **チーム別統計（チームID順）**: チームID、チーム名、最終順位、勝利数、敗北数、引き分け数、総試合数、得点、失点、得失点差、勝率(%)

#### 🎯 **エクスポートファイルの特徴**
- **UTF-8 BOM付き**: Excelで日本語が正しく表示される
- **セクション分離**: `#` で始まるコメント行のタイトルで視覚的に整理
- **日本語ラベル**: 理解しやすい項目名
